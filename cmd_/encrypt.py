...
def execute(argv: [str]):
	...
	if os.path.isfile(args.path):
		encrypt_file(args.path, args.should_remove)
	elif args.should_zip and os.path.isdir(args.path):
		zip_dir_then_encrypt(args.path, args.should_remove)
	elif os.path.isdir(args.path):
		encrypt_dir(args.path, args.should_remove)

def encrypt_file(target_path: Path, should_remove: bool):
	...
	lib.encryptor.encrypt(key, salt, target_path)
...
def zip_dir_then_encrypt(target_path: Path, should_remove: bool):
	...
	progress.set_total_size(target_size)
	key, salt = derive_key(password, None)
	lib.encryptor.encrypt(key, salt, zp)
	os.remove(zp)

	if should_remove:
		shutil.rmtree(target_path)

def encrypt_dir(target_path: Path, should_remove: bool):
	password = get_password(ENCRYPT_MODE)
	...
	for f in ff:
		...
		if os.path.exists(f_out_name): # Overwrite error
			raise Exception(f_out_name + ' already exists')
		lib.encryptor.encrypt(key, salt, f)
		if should_remove:
			os.remove(f)